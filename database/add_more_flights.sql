-- Script para agregar 30 vuelos adicionales a cada aerolínea
-- Ejecutar en Supabase SQL Editor
-- Este script agrega 150 vuelos adicionales (30 por cada aerolínea)

-- Obtener IDs de aerolíneas y generar vuelos adicionales
DO $$
DECLARE
    avianca_id UUID;
    latam_id UUID;
...
BEGIN
    ...
    -- Validar que las aerolíneas existan
    IF avianca_id IS NULL OR latam_id IS NULL OR viva_id IS NULL OR easyfly_id IS NULL OR wingo_id IS NULL THEN
        RAISE EXCEPTION 'No se encontraron todas las aerolíneas necesarias. Ejecuta primero seed_airlines_and_flights.sql';
    END IF;

    -- Generar 30 vuelos adicionales para Avianca (IDs 31-60)
    FOR i IN 31..60 LOOP
        ...
        -- Fecha en los próximos 90 días (variar más las fechas)
        ...
        -- Precios base según categoría (mantener mismo rango)
        ...
        flight_id := 'AVA-' || LPAD(i::text, 4, '0') || '-' || TO_CHAR(flight_date, 'YYYYMMDD') || '-' || LPAD((floor(random() * 1000))::text, 3, '0');
    ...
    -- Generar 30 vuelos adicionales para LATAM (IDs 31-60)
    FOR i IN 31..60 LOOP
        city_from := cities[1 + floor(random() * array_length(cities, 1))::int];
        LOOP
            city_to := cities[1 + floor(random() * array_length(cities, 1))::int];
        ...
        flight_id := 'LCO-' || LPAD(i::text, 4, '0') || '-' || TO_CHAR(flight_date, 'YYYYMMDD') || '-' || LPAD((floor(random() * 1000))::text, 3, '0');
    ...
    -- Generar 30 vuelos adicionales para Viva Air (IDs 31-60)
    FOR i IN 31..60 LOOP
        city_from := cities[1 + floor(random() * array_length(cities, 1))::int];
        LOOP
            city_to := cities[1 + floor(random() * array_length(cities, 1))::int];
        ...
        flight_id := 'VVC-' || LPAD(i::text, 4, '0') || '-' || TO_CHAR(flight_date, 'YYYYMMDD') || '-' || LPAD((floor(random() * 1000))::text, 3, '0');
    ...
    -- Generar 30 vuelos adicionales para EasyFly (IDs 31-60)
    FOR i IN 31..60 LOOP
        city_from := cities[1 + floor(random() * array_length(cities, 1))::int];
        LOOP
            city_to := cities[1 + floor(random() * array_length(cities, 1))::int];
        ...
        flight_id := 'EFY-' || LPAD(i::text, 4, '0') || '-' || TO_CHAR(flight_date, 'YYYYMMDD') || '-' || LPAD((floor(random() * 1000))::text, 3, '0');
    ...
    -- Generar 30 vuelos adicionales para Wingo (IDs 31-60)
    FOR i IN 31..60 LOOP
        city_from := cities[1 + floor(random() * array_length(cities, 1))::int];
        LOOP
            city_to := cities[1 + floor(random() * array_length(cities, 1))::int];
        ...
        flight_id := 'RPA-' || LPAD(i::text, 4, '0') || '-' || TO_CHAR(flight_date, 'YYYYMMDD') || '-' || LPAD((floor(random() * 1000))::text, 3, '0');
    ...
    RAISE NOTICE 'Se han agregado 150 vuelos adicionales exitosamente (30 por cada aerolínea)';
END $$;
...
SELECT 
    ...
    AVG(f.price_cop)::INTEGER as precio_promedio,
    COUNT(DISTINCT f.category) as categorias_distintas,
    COUNT(DISTINCT f.from_city) as ciudades_origen,
    COUNT(DISTINCT f.to_city) as ciudades_destino
FROM airlines a
...
-- Mostrar algunos vuelos de ejemplo recién agregados
SELECT 
    f.id,
    f.airline,
    f.from_city,
...
WHERE a.status = 'activa'
    AND f.id LIKE '%-0031-%'  -- Vuelos con ID 31 (primeros del nuevo lote)
ORDER BY f.airline, f.date
LIMIT 20;
